...
import SingleQualityTable from "./SingleQualityTable/SingleQualityTable";
...
const QualityReport = () => {
  ...
  const getChallanData = () => {
    ...
    getChallans(collection(db, "Challan")).then((docs) => {
      setChallans(docs);
  ...
  useEffect(() => {
    ...
    let qualitynames = qualitylist.map((item, i) => {
      var name = item.name;
      var totalValue = item.data.reduce((result, item) => {
      ...
      var challanValues = challanQualities.reduce((result, item) => {
        if (
          item.name == name &&
          item.type == "Receipt" &&
          item.num_of_taka != undefined
        )
          return result + item.num_of_taka;
        if (
          item.name == name &&
          item.type == "Dispatch" &&
          item.num_of_taka != undefined
  ...
  if (filterList.length > 0)
    result = qualitylist.filter((item) =>
      filterList.some((filterItem) => filterItem.value === item.name)
    );
  var totalSum = qualityNames.reduce((result, item) => {
    return result + item.qualityTotal;
  }, 0);
  var todaysDate = new Date();
  return isRefresh ? (
    <h2 className="text-center">Refreshing</h2>
  ) : (
    <Container fluid>
      ...
      <Row className="pt-5 quality-report">
        <div>
          <h6>
            Date: {normalDateFormat(todaysDate)}
            <br /> Time: {todaysDate.getHours()}:{todaysDate.getMinutes()}
          </h6>
        </div>
        {result?.map((item, i) => {
          var singleQualityName = item.name;

          var totalNum = qualityNames.find(
            (quality) => quality.value === item.name
          );
          var qualityDataList = item.data.map((item) => {
            ...
            var finalStock = challanQualities.reduce((result, item) => {
              ...
              if (
                item.name == singleQualityName &&
                item.value == value &&
                item.type == "Receipt"
              )
                return result + num_of_taka;
              if (
                item.name == singleQualityName &&
                item.value == value &&
                item.type == "Dispatch"
          ...
          return (
            <SingleQualityTable
              ...
              newArr={newArr}
              singleQualityName={singleQualityName}
              totalNum={totalNum}
              key={i}
              minVal={minVal}
            />
